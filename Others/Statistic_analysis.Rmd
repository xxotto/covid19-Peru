...
```{r}
vnd_xdep = read.csv('Data/TOTAL_vacunados_y_fallecidos_x_departamento.csv')
```

## 3. Linear model
```{r}
lm <- lm(tasa_mortalidad ~ vac_porcentaje, data = vnd_xdep)

```

## 4. Linear model
```{r}
lm_graph<-ggplot(vnd_xdep, aes(x=vac_porcentaje, y=tasa_mortalidad)) +
